...
def test1(scope):
    scope.set_header('Test user approve application')
    ...
    presented_user = user_ids[0][0]
    admin_user = user_ids[1][0]
    cur.execute('''
        ...
        values
        ({user_id1}, 'Mickey', 'Doe', 'mickey.doe@gmail.com', 'MCDMCK01A01F205Z', '1234567890', 'FAR', to_date('2024-01-01', 'YYYY-MM-DD') - interval '1 day')
        returning presentation_id
    '''.format(user_id1=presented_user))
    #get all returned ids
    presentation_id = cur.fetchone()[0]
    cur.execute('''
        insert into person (tax_code, account_id, given_name, family_name, email, person_role) values
        ('SMNMRZ01A01F205Z', {user_id}, 'Sam', 'Marz', 'sam.marz@gmail.com', 'WA')
        returning global_id
    '''.format(user_id=admin_user))

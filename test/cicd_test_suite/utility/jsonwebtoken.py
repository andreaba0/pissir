...
class JWTRegistry: 
    ...
    def generate(header, payload):
        ...
        cacheKeyPlain = f"{strHeader}.{strPayload}"
        cacheKey = hashlib.sha256(cacheKeyPlain.encode()).hexdigest()
        if cacheKey in JWTRegistry.cache:
            return JWTRegistry.cache[cacheKey]
        
        token = jwt.encode(payload, KeyRegistry.get(header["kid"]), algorithm=header["alg"], headers=header)
        JWTRegistry.cache[cacheKey] = token
        return token
...
'''
    ...
    def get(key_id):
        return KeyRegistry.keys[key_id]